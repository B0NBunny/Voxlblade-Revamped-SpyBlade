...
function boxBase:Update()
    ...
    if self.IsEnabled and 
        (type(self.IsEnabled) == "string" and
        not _G.savedsettings[self.IsEnabled] and
        not ESP[self.IsEnabled] or 
        type(self.IsEnabled) == "function" and
        not self:IsEnabled())
	then
        allow = false
    end
    ...
    if _G.savedsettings[self.RangeValue] then
        local currentdistance = (cam.CFrame.p - cf.p).magnitude
        if _G.savedsettings[self.RangeValue] < currentdistance then
            allow = false
        end
    end
    ...
    if _G.savedsettings[self.IsBoxEnabled] then --Boxes
    ...
    if _G.savedsettings[self.IsNameEnabled] then --Names
    ...
    if _G.savedsettings[self.IsHealthEnabled] then --Health
        local TagPos, Vis5 = WorldToViewportPoint(cam, locs.TagPos.p)
    ...
    if _G.savedsettings[self.IsDistanceEnabled] then --Distance
        local TagPos, Vis5 = WorldToViewportPoint(cam, locs.TagPos.p)
    ...
    if _G.savedsettings[self.IsTracerEnabled] then --Tracers
